(function () {
	GameEvents.Subscribe("overthrow_item_choice", ShowItemChoice);
})();
...
function TickItemTime() {
	if ($('#item_choice_container').style.visibility == 'visible') {
		...
		if ($('#remaining_time').value <= 0) {
			MakeItemChoice( Math.floor(Math.random() * 4) + 1)
		} else {
...
function MakeItemChoice(slot) {
	...
	GameEvents.SendCustomGameEventToServer("overthrow_item_choice_made", {owner_entindex: owner_index, item: item});
